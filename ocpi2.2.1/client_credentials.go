...
func (c *ClientConn) RegisterCredentials(ctx context.Context, req Credential) (*ocpi.Response[Credential], error) {
	...
	if err := c.do(ctx, c.tokenA, http.MethodGet, c.versionUrl, nil, &versionResponse); err != nil {
		return nil, err
	}
...
func (c *ClientConn) UpdateCredentials(ctx context.Context, credentialWithTokenB Credential, storeCallback func(VersionDetails) error) (*ocpi.Response[Credential], error) {
	var versionResponse ocpi.Response[ocpi.Versions]
	if err := c.do(ctx, c.tokenA, http.MethodGet, c.versionUrl, nil, &versionResponse); err != nil {
		return nil, err
	}
	...
	if err := c.do(ctx, c.tokenA, http.MethodGet, version.URL, nil, &versionDetailsResponse); err != nil {
		return nil, err
	}
	...
	return c.PutCredentials(ctx, credentialWithTokenB)
}
