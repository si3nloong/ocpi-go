...
type Client interface {
	...
	GetLocations(ctx context.Context, params ...GetLocationsParams) (*ocpi.PaginatedResponse[DateTime, Location], error)
	GetLocation(ctx context.Context, locationID string) (*ocpi.Response[DateTime, Location], error)
	GetClientOwnedLocation(ctx context.Context, countryCode string, partyID string, locationID string) (*ocpi.Response[DateTime, Location], error)
	PutClientOwnedLocation(ctx context.Context, countryCode string, partyID string, locationID string, location Location) (*ocpi.Response[DateTime, any], error)
	PatchClientOwnedLocation(ctx context.Context, countryCode string, partyID string, locationID string, location PartialLocation) (*ocpi.Response[DateTime, any], error)
	...
	CancelReservation(ctx context.Context, req CancelReservation) (*ocpi.Response[DateTime, CommandResponse], error)
	UnlockConnector(ctx context.Context, req UnlockConnector) (*ocpi.Response[DateTime, CommandResponse], error)
	SetSessionChargingPreferences(ctx context.Context, sessionID string) (*ocpi.Response[DateTime, ChargingPreferencesResponse], error)
...
func (c *ClientConn) do(ctx context.Context, method, endpoint string, src, dst any) error {
	...
	if scanner, ok := dst.(ocpi.HeaderScanner); ok {
		if err := scanner.ScanHeader(res.Header); err != nil {
			return fmt.Errorf(`ocpi221: unable to scan headers: %w`, err)
		}
	}
